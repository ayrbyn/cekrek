...
export default function Pricing() {
  return (
    <section id="pricing" className="py-24 bg-gradient-to-b from-blue-950 to-blue-900 overflow-hidden">
      <div className="max-w-5xl mx-auto px-4">
        <Reveal className="text-center mb-16">
          ...
          <p className="text-blue-300 mt-3 text-sm max-w-xl mx-auto">
        ...
        <div className="flex flex-row gap-5 items-stretch justify-center flex-wrap md:flex-nowrap">
          {PRICING.map((p, i) => (
            ...
            >
              <div
                className={`flex-1 flex flex-col rounded-3xl overflow-hidden transition-all duration-300 hover:-translate-y-2 active:-translate-y-2 hover:shadow-xl active:shadow-xl hover:shadow-blue-900/50 active:shadow-blue-900/50 ${
              ...
              >
                {p.popular && (
                  <div className="bg-yellow-400 py-2 flex items-center justify-center gap-2">
                    <Star size={14} color="#1E3A8A" fill="#1E3A8A" />
                    <span className="text-blue-900 text-xs font-black uppercase tracking-widest">
                      Paling Populer
                    </span>
                  </div>
                )}
                ...
                <div className="bg-white flex-1 px-8 py-7 flex flex-col justify-between">
                  <ul className="space-y-3 mb-8">
                    {p.features.map((f) => (
                      <li key={f} className="flex items-start gap-3 text-sm text-gray-600">
                        <CheckCircle size={15} color="#1D4ED8" className="flex-shrink-0 mt-0.5" />
                        {f}
                      </li>
                    ))}
                  </ul>
                  <a
                    ...
                    style={{
                      background: p.popular ? "#FACC15" : "#1E3A8A",
                      color: p.popular ? "#1E3A8A" : "#fff",
                    }}
                  >
